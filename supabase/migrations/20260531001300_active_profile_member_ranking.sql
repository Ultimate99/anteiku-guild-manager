-- Anteiku Guild Manager - Member Ranking active-profile identity
-- Migrates only the member-safe ranking viewer identity/scope to the
-- selected active profile. Ranking order, member-safe payload shape, public
-- profile links, and CP privacy remain unchanged.
...
declare
  ...
  actor_guild_id uuid;
begin
  if actor_auth_id is null or actor_id is null then
